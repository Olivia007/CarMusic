...
- (instancetype)initWithDelegate:(id<WZDraggableSwitchHeaderViewDelegate>)delegate frame:(CGRect)frame normalStateHeight:(CGFloat)normalStateHeight heightCanTriggerSwitch:(CGFloat)heightCanTriggerSwitch
{
    if (self = [super initWithFrame:frame]) {
        ...
        layer.frame = CGRectMake((50 - WZ_ARROW_IMAGE_HEIGHT)/2,
                                 frame.size.height - 50 + (50 - WZ_ARROW_IMAGE_HEIGHT)/2,
        ...
        self.statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(layer.frame.size.width + 2 * WZ_ARROW_IMAGE_HEIGHT,frame.size.height - 50,frame.size.width,50.0f)];
        ...
        self.normalColor = WZ_HEADER_VIEW_BACKGROUND_COLOR;
...
{
    if (!_normalColor) {
        _normalColor = WZ_HEADER_VIEW_BACKGROUND_COLOR;
    }
    return _normalColor;
}
    ...
    switch (astate) {
        case WZDraggableSwitchStateNormal:
            if (self.state == WZDraggableSwitchStateDeepDragging || self.state == WZDraggableSwitchStateShallowDragging) {
                [self moveInHeaderViewWithCommonTransitionSubtypes:kCATransitionFromTop];
        ...
        case WZDraggableSwitchStateShallowDragging:
            if (self.state == WZDraggableSwitchStateNormal || self.state == WZDraggableSwitchStateDeepDragging) {
                if (self.state == WZDraggableSwitchStateNormal) {
                    [self moveInHeaderViewWithCommonTransitionSubtypes:kCATransitionFromBottom];
                }
...
- (void)moveInHeaderViewWithCommonTransitionSubtypes:(NSString *)subtype{
    ...
    [[self layer] addAnimation:animation forKey:@"MOVEIN_ANIMATION"];
}
